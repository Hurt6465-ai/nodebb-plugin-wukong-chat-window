...
function signWukongToken(uid) {
  return crypto
    .createHash('sha256')
    .update(`wk:${uid}:${config.WK_SECRET_KEY}`)
    .digest('hex');
}
...
async function smartPost(pathList, payload, timeout = 5000) {
  ...
  for (const path of pathList) {
    ...
    try {
      const data = await requestJson(`${config.WK_HOST}${path}`, {
        method: 'POST',
        headers: {
  ...
  throw lastErr || new Error('WuKong request failed');
}
...
async function syncHistory(loginUid, channelId, limit = 20, startMessageSeq = 0) {
  return await smartPost(
    ['/v1/channel/messagesync', '/channel/messagesync'],
    {
...
async function syncConversation(loginUid, version = 0, msgCount = 1) {
  return await smartPost(
    ['/conversation/sync', '/v1/conversation/sync'],
    {
      uid: String(loginUid),
